...
pca.text.file = "pca_values_lncRNA.txt"
...
for (group in plot.groups){
	...
	pca.file = paste("pca_by_",group,"_lncRNA.png",sep="")
	png(file=pca.file)
	...
	box.file = paste("box_plot_by_",group,"_lncRNA.png",sep="")
	png(file=box.file)
	boxplot(normalized.mat, col=labelColors, xaxt='n')
	legend("topleft",legend=groups,col=color.palette,  pch=19)
	dev.off()

	cluster.file = paste("cluster_by_",group,"_lncRNA.png",sep="")
	...
	hist.file = paste("density_by_",group,"_lncRNA.png",sep="")
	png(file = hist.file)
