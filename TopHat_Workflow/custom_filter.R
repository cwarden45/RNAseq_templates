#I recommend using more liberal thresholds for the filtering category
#such as |fold-change| > 1.2 and setting FDR cutoff to 1 (to only used unadjusted p-value < 0.05)
...
filter.dir.table = read.table(filter.dir.file, sep="\t", head=T)

matched.genes = normal.dir.table$symbol[match(filter.dir.table$symbol, normal.dir.table$symbol, nomatch=0)]
normal.dir.table = normal.dir.table[match(matched.genes, normal.dir.table$symbol, nomatch=0), ]
filter.dir.table = filter.dir.table[match(matched.genes, filter.dir.table$symbol, nomatch=0), ]

status = rep("No Change", times=length(matched.genes))
up.genes = matched.genes[(normal.dir.table$status == normal.upID) & (filter.dir.table$status != filter.upID)]
status[(normal.dir.table$status == normal.upID) & (filter.dir.table$status != filter.upID)]=overall.upID
print(paste("Up-Regulated: ",length(up.genes),sep=""))
down.genes = matched.genes[(normal.dir.table$status == normal.downID) & (filter.dir.table$status != filter.downID)]
status[(normal.dir.table$status == normal.downID) & (filter.dir.table$status != filter.downID)]=overall.downID
...
filter.stats = filter.dir.table[3:ncol(filter.dir.table)]
colnames(filter.stats) = paste(filter.cat, colnames(filter.stats),sep=".")

output.table = data.frame(gene.info, normal.stats, filter.stats, overall.status=status)
write.table(output.table, intersect.output.table, sep="\t", quote=T, row.names=F)
...
if (goseq.flag){
	...
	bias.file = goseq.bias.down.file
	png(bias.file)
	pwf=nullp(deg,genome,"geneSymbol")
	GO.wall=goseq(pwf,genome,"geneSymbol")
	GO.wall = data.frame(GO.wall, overenrichment.fdr=p.adjust(GO.wall$over_represented_pvalue, "fdr"))
