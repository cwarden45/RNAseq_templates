...
previousAlnFolder = "../../[genome]_TopHat_Alignment"
...
fileResults = os.listdir(previousAlnFolder)
...
for file in fileResults:
	...
	if bamResult:
		...
		if (sample not in finishedSamples):
			...
			text = text + "#$ -M "+email+"\n"
			text = text + "#$ -m bea\n"
			text = text + "#$ -N tophatMut"+str(jobCount)+"\n"
			text = text + "#$ -q short.q\n"
			text = text + "#$ -l vf="+re.sub("g","G",java_mem)+"\n"
			text = text + "#$ -j yes\n"
			text = text + "#$ -o tophatMut"+str(jobCount)+".log\n"
			text = text + "#$ -cwd\n"
			text = text + "#$ -V\n"
			...
			tophatBam = previousAlnFolder + "/" + file																		
			rgBam = outputSubfolder + "/rg.bam"
			text = java + " -Xmx" + java_mem + " -Djava.io.tmpdir="+ tempDir + " -jar "+jar_path+"picard-tools-2.5.0/picard.jar AddOrReplaceReadGroups I=" + tophatBam + " O=" + rgBam + " SO=coordinate RGID=1 RGLB=RNA-Seq RGPL=Illumina RGPU=COH RGSM=" + sample + "\n"
			outHandle.write(text)	
