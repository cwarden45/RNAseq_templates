...
normalized.table = read.table(gene.tpm.file, sep="\t", header=T)
normalized.mat = log2(normalized.table[,match(userID, names(normalized.table))] + min.expression)
...
write.table(quantiles, "log2_tpm_quantiles.txt",sep="\t", quote=F, row.names=F)
			...
			if(i == 1)
				{
					...
					expr <- den$x
					freq <- den$y
					plot(expr, freq, type="l", xlab = paste("Log2(TPM + ",min.expression,") Expression",sep=""), ylab = "Density",
