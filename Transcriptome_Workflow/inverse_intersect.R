...
intersect.output.table = "switch_dir.txt"
intersect.output.heatmap = "switch_dir.png"
intersect.output.heatmap2 = "switch_dir_no_cluster.png"
goseq.up.file = "switch_goseq_up.txt"
goseq.down.file = "switch_goseq_down.txt"
...
if(heatmap.flag){
	...
	expression.genes = as.character(expression.table$Gene.Name)
	...
	temp.tpm = heatmap.deg.table[,match(sample.label, names(heatmap.deg.table))]
	...
	if(length(plot.groups) > 1){
		...
		png(file = intersect.output.heatmap2)
		heatmap.2(std.expr, col=colorpanel(33, low="blue", mid="black", high="red"), density.info="none", key=TRUE,
					 RowSideColors=labelColors, trace="none", margins = c(5,15), Rowv=F, dendrogram="column")
		dev.off()
	}#end else
}#end heatmap code
